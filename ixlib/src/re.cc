...
static char *RegexPlainText[] = {
    N_("Invalid quantifier"),    N_("Unbalanced backreference"),     N_("Invalid escape sequence"),
    N_("Invalid backreference"), N_("Unterminated character class"), N_("Unable to match without expression"),
};

// regex_exception ------------------------------------------------------------
regex_exception::regex_exception(TErrorCode error, char const *info, char *module, TIndex line)
    : base_exception(error, info, module, line, "RE")
{
}

const char *regex_exception::getText() const
{
    return _(RegexPlainText[Error]);
}

// regex_string::class_matcher ------------------------------------------------
regex_string::class_matcher::class_matcher() : Negated(false)
{
    MatchLength = 1;
}

regex_string::class_matcher::class_matcher(string const &cls) : Negated(false)
{
    MatchLength = 1;
...
regex_string::special_class_matcher::special_class_matcher(type tp) : Type(tp)
{
    MatchLength = 1;
}
...
bool regex_string::special_class_matcher::match(backref_stack &brstack, string const &candidate, TIndex at)
{
    ...
    {
    case DIGIT:
        result = !!isdigit(candidate[at]);
        break;
    case NONDIGIT:
        result = !isdigit(candidate[at]);
        break;
    case ALNUM:
        result = !!isalnum(candidate[at]);
        break;
    case NONALNUM:
        result = !isalnum(candidate[at]);
        break;
    case SPACE:
        result = !!isspace(candidate[at]);
        break;
    case NONSPACE:
        result = !isspace(candidate[at]);
        break;
    ...
    for (TIndex index = from; index < candidate.size();)
    {
        ...
        if (ParsedRegex->match(BackrefStack, candidate, index))
        {
            ...
            for (TIndex i = 0; i < backrefs; i++)
                tempreplacement = findReplace(tempreplacement, XSTRRE_BACKREF + unsigned2dec((unsigned)i),
...
regex_string::matcher *regex_string::parseRegex(string const &expr)
{
    ...
    size_t index = 0;
    matcher *firstobject = nullptr, *lastobject = NULL;
    ...
    while (index < expr.size())
    {
        ...
        case XSTRRE_LITERAL:
        {
            ...
            else
            {
                ...
                {
                case 'd':
                    object = new special_class_matcher(special_class_matcher::DIGIT);
                    break;
                case 'D':
                    object = new special_class_matcher(special_class_matcher::NONDIGIT);
                    break;
                case 'w':
                    object = new special_class_matcher(special_class_matcher::ALNUM);
                    break;
                case 'W':
                    object = new special_class_matcher(special_class_matcher::NONALNUM);
                    break;
                case 's':
                    object = new special_class_matcher(special_class_matcher::SPACE);
                    break;
                case 'S':
                    object = new special_class_matcher(special_class_matcher::NONSPACE);
                    break;
                default:
                    object = new sequence_matcher(string(1, ch));
                }
            }
            break;
        }
        ...
        {
            size_t classend = expr.find(XSTRRE_CLASSSTOP, index);
        ...
        {
            matcher *parsed = nullptr;

            TSize brlevel = 1;
            for (size_t searchstop = index; searchstop < expr.size(); searchstop++)
            {
                if ((expr[searchstop] == XSTRRE_BACKREFSTART) && (expr[searchstop - 1] != XSTRRE_LITERAL))
                    brlevel++;
                if ((expr[searchstop] == XSTRRE_BACKREFSTOP) && (expr[searchstop - 1] != XSTRRE_LITERAL))
                {
                    brlevel--;
                    if (brlevel == 0)
                    {
            ...
            matcher *searchlast = parsed, *foundlast = nullptr;
            while (searchlast)
            {
        ...
        {
            if (quantified)
                quantifier = parseQuantifier(expr, index);
            if (quantifier)
            {
                quantifier->setQuantified(object);
                if (lastobject)
                    lastobject->setNext(quantifier);
                else
                    firstobject = quantifier;
            }
            else
            {
                if (lastobject)
...
regex_string::quantifier *regex_string::parseQuantifier(string const &expr, size_t &at)
{
...
bool regex_string::isGreedy(string const &expr, size_t &at)
{
    if (at == expr.size())
        return true;
