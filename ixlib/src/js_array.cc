...
// js_array -------------------------------------------------------------------
js_array::js_array(TSize size)
{
    Array.resize(size);

    ref_ptr<value> null = javascript::makeNull();
...
ref_ptr<javascript::value> js_array::duplicate()
{
    ref_ptr<value> result = new js_array(*this);

    return result;
}

ref_ptr<javascript::value> js_array::lookup(string const &identifier)
{
...
ref_ptr<javascript::value> js_array::subscript(value const &index)
{
...
ref_ptr<javascript::value> js_array::callMethod(string const &id, parameter_list const &parameters)
{
    if (id == "pop" && parameters.size() == 0)
    {
        if (Array.size() == 0)
            return javascript::makeNull();
        else
        {
            ref_ptr<value> back = Array.back();
            Array.pop_back();
            return back;
        }
    }
    ...
    else if (id == "shift" && parameters.size() == 0)
    {
        ...
        {
            ref_ptr<value> front = Array.front();
...
ref_ptr<value> &js_array::operator[](TIndex idx)
{
    if (idx >= Array.size())
...
void js_array::push_back(ref_ptr<value> val)
{
    Array.push_back(val);
}

// js_array_constructor -------------------------------------------------------
ref_ptr<javascript::value> js_array_constructor::duplicate()
{
    // array_constructor is not mutable
    return this;
}

ref_ptr<javascript::value> js_array_constructor::construct(parameter_list const &parameters)
{
    if (parameters.size() == 0)
        return makeArray();
    else if (parameters.size() == 1)
        return makeArray(parameters[0]->toInt());
